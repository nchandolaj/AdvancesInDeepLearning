...
NVIDIA’s 2026 flagship uses a **Dual-Die Design**. Two massive silicon dies are connected by a 10 TB/s interface, appearing to software as a single unified chip.
...
The TPU is not a GPU; it doesn't have "Warps" or "Threads" in the traditional sense. It uses a **Systolic Array**.
* **The MXU:** Data flows through a 256x256 grid of logic gates like a heartbeat (systolic). Each "beat" performs thousands of multiplications, making it incredibly efficient for the matrix math found in Transformers.
* **Optical Switching (OCS):** Google uses mirrors and light to interconnect up to 9,000+ TPUs in a "Pod," allowing them to reconfigure the supercomputer’s wiring instantly via software.
